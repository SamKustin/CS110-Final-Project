...
class StoryMode:
    ...
    def __renderPeach(self, peach):
        ...
        self.__handleCastleCollision(peach, peachRect)
    ...
    def __handleCastleCollision(self, peach, peachRect):
    ...
    def runStoryMode(self, peach):
        while not self.__completed:
            ...
            if peach.__x in startingPointsX and peach.__y in startingPointsY:
                self.__renderSpeechBubble()
                self.__renderBowser()
                self.__renderMario()
                self.__writeMessage1()
                self.__writeMessage2()

            self.__renderGround()
            self.__renderCastle()

            # associates the sprite with the image in updateSprite()
            peach.__updateSprite(peach)
